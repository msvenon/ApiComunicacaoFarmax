...
uses UPrincipalRefresher, uDmRefresher;
...
begin
    ...
    Posicao := Pos(':', DmRefresher.SQLConnection1.Params.Values['Database']);
    ServerName := Copy(DmRefresher.SQLConnection1.Params.Values['Database'], 1, Posicao - 1);
    DatabaseName := Copy(DmRefresher.SQLConnection1.Params.Values['Database'], Posicao + 1, Length(DmRefresher.SQLConnection1.Params.Values['Database']));
...
begin
  ...
  Try
    Cd_Evento := 0;
    Cd_Evento := DmRefresher.GravaEventos(Cd_Evento, 'Fazendo backup no inicio do dia (Refresher)', True, False);
    ...
    Cd_Evento := DmRefresher.GravaEventos(Cd_Evento, 'Fazendo backup no inicio do dia (Refresher)', False, False);
  Except
      IBBackup.Active := False;
      Application.ProcessMessages;
      bSucesso := 'N';
    Cd_Evento := DmRefresher.GravaEventos(Cd_Evento, 'Fazendo backup no inicio do dia (Refresher)', False, True);
  End;
end;
